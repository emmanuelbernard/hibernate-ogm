...
Hibernate OGM uses the +_id+ field of a MongoDB document to store the identifier columns mapped in the entity.

An identifier type may be one of the <<mongodb-built-in-types>> as well as an embedded class.
...
The preferable one, Hibernate OGM will automatically assign an id generated during insert.
...
[source, JSON]
----
{ "_id" : "hibernate_sequence", "next_val" : 8 }
----
====
...
Embedded identifiers are stored as embedded documents.
Hibernate OGM will always store the +@EmbeddedId+ property into a +_id+ document field
(the recommended approach in MongoDB) ignoring the name of the property.
...
(see <<mongodb-one-collection-strategy-join-table>>) 
